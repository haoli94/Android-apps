...
//			System.out.print(point);
	...
	*/
	public int[] getSkirt() {
		int[] lowestY = new int[]{Integer.MAX_VALUE,Integer.MAX_VALUE,Integer.MAX_VALUE,Integer.MAX_VALUE};
		for (int i=0; i < body.length;i++) {
			int index = body[i].x;
			if (lowestY[index] > body[i].y) {
				lowestY[index] = body[i].y;
			}
		}
		skirt = new int[width];
		for (int j=0;j<width;j++) {
			skirt[j] = lowestY[j];
//			System.out.println(lowestY[j]);
		}
		return skirt;
	}
	...
	public Piece computeNextRotation() {
		String nextRotate	= new String();
		for (TPoint point : body) {
			nextRotate += height-1-point.y + " " + point.x + " ";
		}
		return new Piece(nextRotate); // YOUR CODE HERE
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		boolean equal = true;
		for (TPoint point1 : body) {
			boolean pointEqual = false;
			for (TPoint point2 : other.body) {
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece prev;
        Piece nextShape = root.computeNextRotation();
        root.next = nextShape;
        prev = nextShape;
        while (!root.equals(nextShape)) {
            prev = nextShape;
            nextShape = prev.computeNextRotation();
            prev.next = nextShape;
        }
        prev.next = root;
